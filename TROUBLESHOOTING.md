...
```mermaid
flowchart TD
    Start([🔍 Start Here]) --> Q1{Dashboard loads?}
    
    Q1 -->|No| A1[Check Vercel deployment<br/>& browser console]
    Q1 -->|Yes| Q2{Convex connected?}
    
    Q2 -->|No| A2[Check CONVEX_URL<br/>in index.html<br/>Run: npx convex dev]
    Q2 -->|Yes| Q3{Data flowing?}
    
    Q3 -->|Yes| Success([✅ All Good!])
    Q3 -->|No| Q4{Python bridge<br/>running?}
    
    Q4 -->|No| A3[Start: python<br/>backend/maindata.py]
    Q4 -->|Yes| A4[Check Ably API key<br/>& channel name]
    
    A1 --> Fix1[Fix deployment issues]
    A2 --> Fix2[Update config & restart]
    A3 --> Q3
    A4 --> Fix4[Verify Ably config]
```
...
1. **Role restrictions**
   ...
   - `external` role can only view the last session, is capped to 1000 total representative data points, and cannot access Custom Analysis
