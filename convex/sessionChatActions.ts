...
async function generateAnswer(args: {
  ...
  return await generateText({
    ...
    system: [
      ...
      "When a graph materially improves the answer, append at most two fenced ecovolt-chart blocks containing JSON only.",
      "For sector bars or lines use {\"view\":\"sector-series\",\"chart\":\"bar\",\"metrics\":[metric names],\"title\":\"short title\"}; chart may be \"bar\" or \"line\".",
      "For a sector scatter plot use {\"view\":\"sector-scatter\",\"xMetric\":\"metric name\",\"yMetric\":\"metric name\",\"title\":\"short title\"}.",
      "For a full-run line graph use {\"view\":\"timeline\",\"metrics\":[metric names],\"title\":\"short title\"}; the browser resolves and downsamples those fields from its already-loaded telemetry.",
      "Allowed metric names are avgSpeedKmh, maxSpeedKmh, avgPowerW, peakPowerW, energyWh, distanceKm, speedVariationKmh, stoppedPct, and anomalyCount.",
      "Timeline metric names are speed_kmh, power_w, voltage_v, current_a, throttle_pct, brake_pct, vesc_current_a, motor_rpm, motor_temp_c, g_force, alt, and efficiency.",
      "Use no more than two same-unit metrics in a sector-series or timeline chart. Never include chart values, JavaScript, callbacks, HTML, colors, or styling; the browser resolves approved metrics from session evidence.",
      "If the user explicitly asks for a graph or chart and the requested evidence is available, include an ecovolt-chart block.",
