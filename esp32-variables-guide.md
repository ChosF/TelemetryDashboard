...
| `optimal_speed_kmh` | `speed_ms`, `power_w`, `timestamp`; motion/brake context | Best supported flat-road cruising speed from physics-informed RLS or stable observed energy-per-distance evidence |
| `optimal_speed_ms` | same as above | Optimal speed in m/s, constrained to observed speed coverage |
| `optimal_efficiency_km_kwh` | same as above | Predicted or directly observed efficiency at the supported optimal speed |
| `optimal_speed_confidence` | evidence duration, speed coverage, residuals, model uncertainty or cruise stability | Confidence in the current optimal-speed estimate; single-band evidence is capped until multiple speeds are tested |
