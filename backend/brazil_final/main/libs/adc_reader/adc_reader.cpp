...
void ADCReader::configure_adc() {
    ...
    adc1_config_channel_atten(throttle_channel, ADC_ATTEN_DB_11);  // Configure throttle on GPIO 3
    esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_DB_11, ADC_WIDTH_BIT_12, 0, &adc_chars);
    ESP_LOGI(TAG, "ADC configured (voltage, current, throttle).");
}
...
float ADCReader::read_throttle_pct() {
    // Read throttle from GPIO 3 (ADC1_CHANNEL_3)
    // Voltage range: 0.83V (0%) to 3.33V (100%)
    
    uint32_t adc_reading = 0;
    for (int i = 0; i < ADC_SAMPLES; i++) {
        adc_reading += adc1_get_raw(throttle_channel);
    }
    ...
    // Convert voltage to percentage using calibration constants
    // Linear mapping: 0.83V = 0%, 3.33V = 100%
    float throttle_pct = (voltage_v - THROTTLE_MIN_VOLTAGE) / (THROTTLE_MAX_VOLTAGE - THROTTLE_MIN_VOLTAGE) * 100.0f;
    
    // Clamp to valid range [0, 100]
    if (throttle_pct < 0.0f) throttle_pct = 0.0f;
    if (throttle_pct > 100.0f) throttle_pct = 100.0f;
    
    ESP_LOGI(TAG, "Throttle ADC Raw: %d, Voltage: %.3f V, Throttle: %.1f%%", (int)adc_reading, voltage_v, throttle_pct);
