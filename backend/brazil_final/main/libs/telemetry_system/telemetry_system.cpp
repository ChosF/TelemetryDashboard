...
void TelemetrySystem::run() {
    ...
    while (true) {
        ...
        telemetry.data.voltage_v = fabsf(voltage);
        telemetry.data.current_a = fabsf(current);
        telemetry.data.power_w = fabsf(power);
        telemetry.data.energy_j = fabsf(cumulative_energy);
        telemetry.data.distance_m = cumulative_distance;
        
        // Read throttle position from GPIO 3
        telemetry.data.throttle_pct = adc_reader.read_throttle_pct();

        if (mqtt_client.publish(telemetry)) {
            ESP_LOGI(TAG, "Message %d sent. Speed: %.2f m/s, Lat: %.4f, Lon: %.4f, Alt: %.2f, Throttle: %.1f%%",
                telemetry.data.message_id,
                telemetry.data.speed_ms,
                telemetry.data.latitude,
                telemetry.data.longitude,
                telemetry.data.altitude,
                telemetry.data.throttle_pct);
        } else {
